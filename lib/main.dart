...
Future<void> createEngine() async {
  ...
  // Validate appId
  if (appId == 0) {
    AppSettings.showLog("⚠️ Zego App ID not available yet. Skipping engine creation.");
    return;
  }
  
  try {
    AppSettings.showLog("✅ Creating Zego Engine with AppID: $appId (without appSign)");
...
Future<void> stripeInit() async {
  if (CustomCheckInternet.isConnect.value) {
    try {
      final stripeKey = AdminSettingsApi.adminSettingsModel?.setting?.stripePublishableKey 
      ...
      if (stripeKey.isNotEmpty && !stripeKey.contains('your_key_here')) {
        ...
        AppSettings.showLog("⚠️ Stripe key not configured, skipping initialization");
      }
    } catch (e) {
