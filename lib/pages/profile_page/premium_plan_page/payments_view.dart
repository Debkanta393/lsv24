...
class _PaymentViewState extends State<PaymentView> implements IAPCallback {
  ...
  @override
  void initState() {
    print("==== PaymentView INIT ====");
    print("premiumPlanId => ${widget.premiumPlanId}");
    print("productKey => ${widget.productKey}"); // ← must match badgeMap keys
  ...
  String _getBadgeLabel(String productKey) {
    ...
    print("_getBadgeLabel called with: '$productKey'"); // 🔍 debug
    print("result: '${badgeMap[productKey]}'");
    return badgeMap[productKey] ?? '';
  }

  Future<void> _saveBadgeAfterPurchase() async {
    final badge = _getBadgeLabel(widget.productKey);
    print("==== PREMIUM BADGE SAVE ====");
    print("productKey => ${widget.productKey}");
    print("badge => $badge");
    if (badge.isNotEmpty) {
      await Database.onSetPurchasedPlan(widget.productKey, badge);
      print("Badge saved successfully => $badge");
    } else {
      print("No badge mapped for productKey: ${widget.productKey}");
    }
  }
            ...
            RazorPayService().init(
              ...
              callback: () async {
                ...
                await CreatePremiumPlanApi.callApi(
                    Database.loginUserId!, widget.premiumPlanId, "Razorpay");
      ...
      body: Column(
        ...
        children: [
          ...
          Text(
            ...
            style:
                GoogleFonts.urbanist(fontSize: 14, fontWeight: FontWeight.w500),
          ).paddingOnly(left: 20),
          SizedBox(height: SizeConfig.blockSizeVertical * 2),
          (AdminSettingsApi.adminSettingsModel?.setting?.razorPaySwitch ??
                  false)
              ? PaymentItemView(
